...
export default function CustomCalendar2() {
  ...
  const RenderWeeks = ({weeks}) => {
    // const weeks = getWeeksForMonth(selectedDate.format('YYYY'),selectedDate.format('M'));
    return(
  ...
  const listOfThreeMonths = () => {
    let listOfThreeMonths = [];
    listOfThreeMonths.push(weeksForPrevMonth());
    listOfThreeMonths.push(weeksForCurrentMonth());
    listOfThreeMonths.push(weeksForNextMonth());
  ...
  return ( 
    <View style={styles.container}> 
      ...
      <View style={{width: '100%'}}>
        ...
        <FlatList 
          ref={flatListRef}
          data={listOfThreeMonths()}
          renderItem={({item}) => <RenderWeeks weeks={item} />}
  ...
  function currentMonth () {
    ...
    function handleMonthChange(direction) {
      ...
      // setSelectedDate(selectedDate.add(direction, 'months'));
      // setSelectedMonth(selectedDate.format('MMMM YYYY'));
      const newDate = selectedDate.clone().add(direction, 'months');
      setSelectedDate(newDate);
      setSelectedMonth(newDate.format('MMMM YYYY'));
      flatListRef.current.scrollToIndex({ index: 1, animated: false });
